...
fun ManagerSegmentedButtons(
    ...
    Crossfade(targetState = selectedIndex) {
        Row(Modifier.fillMaxWidth().height(IntrinsicSize.Max).padding(8.dp).clip(ManagerComposableShape).background(backgroundColor).padding(3.dp)) {
            options.forEachIndexed { index, option ->
                ...
                Text(
                    ...
                    modifier = Modifier.fillMaxSize().weight(1f).clip(ManagerComposableShape)
